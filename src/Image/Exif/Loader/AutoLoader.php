    ...
    public static function Load( $imageFile, $exifToolPath = null, $exiV2Path = null ): ?ImageInfo
    {
        ...
        if ( !$exifToolLoader->isConfigured() )
        {
            if ( !empty( $exifToolPath ) )
            {
                try
                {
                    $exifToolLoader->configure( [ 'path' => $exifToolPath ] );
                }
                catch ( Throwable $ex )
                {
                    $ex = null;
        ...
        if ( $exifToolLoader->isConfigured() )
        {
            try
            {
                return $exifToolLoader->load( $imageFile );
            }
            catch ( Throwable $ex )
            {
                $ex = null;
        ...
        if ( !$exiV2Loader->isConfigured() && !empty( $exiV2Path ) )
        {
            try
            {
                $exiV2Loader->configure( [ 'path' => $exiV2Path ] );
            }
            catch ( Exception $ex )
            {
                $ex = null;
        ...
        if ( $exiV2Loader->isConfigured() )
        {
            try
            {
                return $exiV2Loader->load( $imageFile );
            }
            catch ( Exception $ex )
            {
                $ex = null;
        ...
        try
        {
            return $phpLoader->load( $imageFile );
        }
        catch ( Exception $ex )
        {
            $ex = null;
        ...
        try
        {
            return $jsonLoader->load( $imageFile );
        }
        catch ( Exception $ex )
        {
            $ex = null;
