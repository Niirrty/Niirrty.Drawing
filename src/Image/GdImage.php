...
use \Niirrty\Drawing\{ ColorTool, ContentAlign, Point, Rectangle, Size };
...
class GdImage extends AbstractImage
{
    ...
    * @return \Niirrty\Drawing\Image\GdImage
    */
    ...
    * @param \Niirrty\Drawing\Size $targetSize Größe des Bildes in dem der Text
   ...
   private function imageTtfPoint2(
      int $size, int $angle, string $fontfile, string $text, ContentAlign $align, Size $targetSize, int $padding ) : Point
   {
   ...
   private function createImageAfterResize( Size $newSize, bool $internal )
   {
      ...
      \imagecopyresampled($dst, $this->r, 0, 0,0, 0, $newSize->width, $newSize->height, $this->getWidth(), $this->getHeight());
   ...
   private function placementToGdImage( $placement ) : ?GdImage
   {
      ...
      if ( $placement instanceof IImage )
      {
         ...
         if ( $placement->isPng() ) { $tmpFile .= '.png'; }
         else if ( $placement->isGif() ) { $tmpFile .= '.gif'; }
         else { $tmpFile .= '.jpg'; }
    ...
    * @return \Niirrty\Drawing\Image\GdImage
    * @throws \Niirrty\ArgumentException If the image type is invalid/unknown.
    */
